...
module.exports.loadResource = function () {
    ...
    //map load img
    for (var i in maps) {
        resourceLoadHandler.upResource(maps[i], 'img', ['load_img'], 'maps/load_img/');
        resourceLoadHandler.upResource(maps[i], 'img', ['country_img'], 'maps/country/');
        resourceLoadHandler.upResource(maps[i].medpack_list, 'img', ['img'], 'maps/medpack/');
    }
    //hero icon img
    for (var i in heros) {
